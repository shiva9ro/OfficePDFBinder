...
def test_restricted_portable_mode_is_enabled_only_by_marker(tmp_path, monkeypatch):
    monkeypatch.setattr(app_module, "_get_runtime_dir", lambda: str(tmp_path))

    assert app_module._is_restricted_portable_mode() is False

    (tmp_path / app_module.RESTRICTED_PORTABLE_MARKER_FILENAME).touch()

    assert app_module._is_restricted_portable_mode() is True
...
def test_normal_mode_office_temp_pdf_uses_default_temp(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(app_module, "_is_restricted_portable_mode", lambda: False)
...
def test_restricted_mode_office_temp_pdf_uses_source_folder(tmp_path, monkeypatch):
    office_path = tmp_path / "shared-folder" / "sample.docx"
    office_path.parent.mkdir()
    office_path.touch()
    monkeypatch.setattr(app_module, "_is_restricted_portable_mode", lambda: True)
...
def test_restricted_mode_window_does_not_create_or_save_settings(
...
def test_restricted_mode_debug_log_does_not_write_file(tmp_path, monkeypatch):
